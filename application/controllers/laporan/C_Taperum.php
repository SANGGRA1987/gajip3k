...
class C_Taperum extends CI_Controller {
    ...
    public function taperum(){
    	...
    	if (isset($_POST['kd_skpd1'])) {
                ...
                $pdf->Cell(160,12,'Unit Kerja',1,0,'C');
                $pdf->Cell(80,6,'JUMLAH PEGAWAI',1,1,'C');
                $pdf->SetX(189);
                $pdf->Cell(15,6,'Gol 1',1,0,'C');
                $pdf->Cell(15,6,'Gol 2',1,0,'C');
                $pdf->Cell(15,6,'Gol 3',1,0,'C');
                $pdf->Cell(15,6,'Gol 4',1,0,'C');
                $pdf->Cell(20,6,'Semua Gol',1,0,'C');
                $pdf->SetY(56);
                $pdf->SetX(269);
                $pdf->Cell(25,12,'Jumlah',1,0,'C');
                $pdf->Cell(35,12,'Keterangan',1,1,'C');

                $pdf->SetFont('Arial','',12); // font dan ukuran font
                $sql = "SELECT a.satkerja,a.nm_satkerja,
                        (select count(tabungan) from public.pegawai b where b.satkerja=a.satkerja and left(b.golongan,1) = '1' ) as gol_1,
                        (select count(tabungan) from public.pegawai b where b.satkerja=a.satkerja and left(b.golongan,1) = '2' ) as gol_2,
                        (select count(tabungan) from public.pegawai b where b.satkerja=a.satkerja and left(b.golongan,1) = '3' ) as gol_3,
                        (select count(tabungan) from public.pegawai b where b.satkerja=a.satkerja and left(b.golongan,1) = '4' ) as gol_4,
                        (select sum(tabungan) from public.pegawai b where b.satkerja=a.satkerja ) as j_taperum
                ...
                foreach ($taperum as $row){
                        ...
                        $j_taperum = $row->j_taperum;
                        $j_taperum = number_format($j_taperum,0,',','.');
                        ...
                        $pdf->Cell(25,6,"$j_taperum",1,0,'C'); 
                        $pdf->Cell(35,6,"",1,1,'C');    
                        
                }
                        
                        $pdf->SetFont('Arial','B',12);
                        $pdf->Cell(170,6,"JUMLAH KESELURUHAN. . . . . . . . . ",1,0);
                        $pdf->Cell(15,6,"$gol_1",1,0);
                        $pdf->Cell(15,6,"$gol_2",1,0);
                        $pdf->Cell(15,6,"$gol_3",1,0);
                        $pdf->cell(15,6,"$gol_4",1,0);  
                ...
                $sql = "SELECT a.satkerja,a.nm_satkerja,
                        ...
                        (select count(tabungan) from public.pegawai b where b.satkerja=a.satkerja and left(b.golongan,1) = '4' ) as gol_4,
                        (select sum(tabungan) from public.pegawai b where b.satkerja=a.satkerja ) as j_taperum
                        from satkerja a where left(a.satkerja,1)<>'0' ";
                $n_gol1=0;
                $n_gol2=0;
                $n_gol3=0;
                $n_gol4=0;
                ...
                foreach ($taperum as $row){
                        ...
                        $jumlah_gol = $gol_1 + $gol_2 + $gol_3 + $gol_4;
                        $j_taperum = $row->j_taperum;
                        $j_taperum1 = number_format($j_taperum,0,',','.');
                        ...
                        $n_jumlah_taperum=$n_jumlah_taperum + $j_taperum;
                        $j_jumlah_taperum = number_format($n_jumlah_taperum,0,',','.');
                        ...
                        $pdf->cell(15,6,"$gol_4",1,0);  
                        $pdf->cell(20,6,"$jumlah_gol",1,0);   
                        $pdf->Cell(25,6,"$j_taperum1",1,0,'R'); 
                        $pdf->Cell(35,6,"",1,1,'C');    
                        
                }
                
                $pdf->SetFont('Arial','B',12);
                $pdf->Cell(170,10,"JUMLAH KESELURUHAN. . . . . . . . . ",1,0);
                $pdf->Cell(15,10,"$j_gol_1",1,0);
                $pdf->Cell(15,10,"$j_gol_2",1,0);
                $pdf->Cell(15,10,"$j_gol_3",1,0);
                $pdf->cell(15,10,"$j_gol_4",1,0);  
                $pdf->cell(20,10,"$j_jumlah_gol",1,0,'R');   
                $pdf->Cell(25,10,"$j_jumlah_taperum",1,0,'R');
