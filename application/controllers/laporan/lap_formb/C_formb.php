...
class C_formb extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		//Do your magic here
		$this->load->model('laporan/lap_formb/M_formb');
	}
	...
	{	
		$data = array(
			'page'	 	=> 'LAPORAN DAFTAR GAJI PEGAWAI P3K',
			'judul'		=> 'LAPORAN DAFTAR GAJI PEGAWAI P3K',
			'deskripsi'	=> 'LAPORAN DAFTAR GAJI PEGAWAI P3K'
		);

		$this->template->views('laporan/lap_formb/V_formb', $data);
	}
	...
	{
		return $this->M_formb->getConfig();
	}

	public function getBulan()
	{
		echo $this->M_formb->getBulan();
	}

	public function getTahun()
	{
		echo $this->M_formb->getTahun();
	}

	public function getSkpd()
	{
		echo $this->M_formb->getSkpd(); 
	}

	public function getUnitSkpd()
	{
		$param = $this->uri->segment(5);
		echo $this->M_formb->getUnitSkpd($param);
	}
	...
	public function cetakLaporan()
	{
		...
		$bluePrint = $this->M_formb->cetakLaporan($data);
		...
		//echo $where;
		 ...
		 $cRet .="
					...
					$query = $this->db->query($sql);
						...
						$xy=0;
			       		...
			       		foreach ($query->result() as $row) {
			                ...
			                $jkk           =$row->jkk;
			                $jkm           =$row->jkm;
			                $bruto         =$gapok+$tistri+$tanak+$umum+$khusus+$tstruk+$tfung+$bulat+$beras+$jkk+$jkm+$askes+$pph;
			                $iwp           =$row->iwp;
			                $sewa          =$row->sewa;
			        	...
			        	$cRet .="
									...
									<td align=\"right\" border=\"1\" width=\"6%\" style=\"font-size:9px;border-left:dashed 1px gray;border-right:dashed 1px gray;border-bottom:dashed 1px gray;border-top:dashed 1px gray\">".number_format($pph,2,',','.')."<br>".number_format($askes,2,',','.')."</br></td>
		        ...
		        $data['prev']= $cRet;
		        $judul  = 'Laporan Daftar Gaji Pegawai P3K' .'-'. $nmskpd .'-'. $nm_skpd_unit;
		        switch ($jenisCetak) {
		        	case 0:
