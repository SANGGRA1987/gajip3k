...
class C_SPM extends CI_Controller {
    ...
    function untuk(){
		...
		$res = $this->M_SPM->untuk($satkerja,$gol1,$gol2);
    	echo json_encode($res);
	}
	...
	function rekening(){
		...
		$gol1 = $this->input->post('gol1');
		$gol2 = $this->input->post('gol2');
		$res = $this->M_SPM->rekening($satkerja,$gol1,$gol2);
    	echo json_encode($res);
	}
	...
	public function saveData(){
		...
		$h =	$this->M_SPM->saveData($header,$status);
		if($h == 1){
				$sukses =	$this->M_SPM->simpan_detail($nomor_spm,$status,$data,$data2,$kd_skpd,$no_spp,$kd_giat,$nm_giat,$no_spd);
					if($sukses){
    ...
    function load_detail_pot(){
    	...
    	$res = $this->M_SPM->load_detail_pot($data);
    	echo json_encode($res);
    }
    ...
    function ambil_total(){
		$kd_skpd = $this->input->post('kd_skpd');
		$no_spm  = $this->input->post('no_spm');
		$res     = $this->M_SPM->ambil_total($kd_skpd,$no_spm);
    	echo json_encode($res);
	}
