...
class C_Pegawai extends CI_Controller {
	...
	{
		$data = array(
			'page' 		=> "Keterangan Gaji",
			'judul'		=> "Keterangan Gaji",
			'deskripsi'	=> "Keterangan Gaji"
		);
		$this->template->views('master/V_info_gaji', $data);
	}
	...
	public function get_fungsional()
	{
		$lccq 		= $this->input->post('q');
		$golongan 	= $this->input->post('golongan');
		$res 		= $this->M_Pegawai->get_fungsional($lccq,$golongan);
		echo json_encode($res);
	}
	...
	{
		$lccq 		= $this->input->post('q');
		$res 		= $this->M_Pegawai->get_khusus($lccq);
		echo json_encode($res);
	}
