...
class C_golongan extends CI_Controller {
	...
	function load_golongan() {
        ...
        $sql = "SELECT CAST(golongan AS int) as golongan, nm_golongan,tnpapua from golongan $where $limit";
        $query1 = $this->db->query($sql);  
        $result = array();
        $ii = 0;
        ...
        { 
            $row[] = array(
                        ...
                        'tnpapua' => number_format($resulte['tnpapua'],2,'.',',')				
                        );
                        $ii++;
        }
