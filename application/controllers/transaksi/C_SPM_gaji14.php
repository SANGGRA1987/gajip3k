...
class C_SPM_gaji14 extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		$this->load->model('transaksi/M_SPM_gaji14');
	}
	...
	public function index()
	{
		$data = array(
			'page' 		=> "SPM GAJI 14",
			'judul'		=> "SPM GAJI 14",
			'deskripsi'	=> "SPM GAJI 14"
		);

		$this->template->views('transaksi/dokumen/V_SPM_gaji14', $data);
	}


	function load_header(){
		$key = $this->input->post('key');
		$res = $this->M_SPM_gaji14->loadHeader($key);
	...
	public function add()
	{
		...
		$this->template->views('transaksi/dokumen/V_Add_SPM_gaji14', $data);
	}
	...
	public function get_satker()
	{
		...
		$res 		= $this->M_SPM_gaji14->get_satker($lccq);
		echo json_encode($res);
	}
	...
	{
		$lccq 		= $this->input->post('q');
		$res 		= $this->M_SPM_gaji14->golongan1($lccq);
		echo json_encode($res);
	}
    ...
    function untuk(){
		...
		$res = $this->M_SPM_gaji14->untuk($satkerja,$gol1,$gol2);
    	echo json_encode($res);
	}
	...
	function rekening(){
		...
		$gol1 = $this->input->post('gol1');
		$gol2 = $this->input->post('gol2');
		$res = $this->M_SPM_gaji14->rekening($satkerja,$gol1,$gol2);
    	echo json_encode($res);
	}
	...
	public function saveData(){
		...
		$h =	$this->M_SPM_gaji14->saveData($header,$status);
		if($h == 1){
				$sukses =	$this->M_SPM_gaji14->simpan_detail($nomor_spm,$status,$data,$data2,$kd_skpd,$no_spp,$kd_giat,$nm_giat,$no_spd);
					if($sukses){
	...
	public function hapus(){
		...
		$sukses = $this->M_SPM_gaji14->hapus($param);
		if( $sukses ){
	...
	function load_detail(){
    	...
    	$res = $this->M_SPM_gaji14->load_detail($data);
    	echo json_encode($res);
    }
    ...
    function load_detail_pot(){
    	...
    	$res = $this->M_SPM_gaji14->load_detail_pot($data);
    	echo json_encode($res);
    }
    ...
    function ambil_total(){
		$kd_skpd = $this->input->post('kd_skpd');
		$no_spm  = $this->input->post('no_spm');
		$res     = $this->M_SPM_gaji14->ambil_total($kd_skpd,$no_spm);
    	echo json_encode($res);
	}
