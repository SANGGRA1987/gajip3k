...
class C_Proses_gaji14 extends CI_Controller {
	public function __construct()
	{
		parent::__construct();
		$this->load->model('utilitas/M_Proses_gaji14');
	}

	public function index()
	{	
		$data = array(
			'page'	 	=> 'Proses Gaji 14 Pegawai P3K',
			'judul'		=> 'Proses Gaji 14 Pegawai P3K',
			'deskripsi'	=> 'Proses Gaji 14 Pegawai P3K'
		);

		$this->template->views('utilitas/V_Proses_gaji14', $data);
	}

	public function getBulan()
	{
		$lccq 	= $this->input->post('q');
		$res = $this->M_Proses_gaji14->getBulan($lccq);
		echo json_encode($res);
	}

	public function getConfig()
	{
		return $this->M_Proses_gaji14->getConfig();
	}
	...
	function proses(){
		...
		$this->db->query("delete from public.pegawai_14");		
        ...
        foreach($query->result_array() as $resulte){
			...
			$umum    			= $resulte['umum'];
			$sewa_rumah     	= 0; //$resulte['sewa'];
			$taperum     		= 0; //$resulte['tabungan'];
			$h_pembayaran    	= 0; //$resulte['hutang'];
			$pot_lain     		= 0; //$resulte['lain'];
			...
			//iwp di 0 kan di gaji 13
			$iwp = 0;

			//$tht = ((3.25/100) * (($gapok1) + ($tistri) + ($tanak)));
			//tht di 0 kan di gaji 13
			$tht = 0;
			...
			// Yang dimunculkan hanya pph di gaji 13
			$ndisc   = ($pph);
			...
			$this->db->query(" insert into public.pegawai_14(nip_lama,nip,nama,nokartu,kota,agama,stskawin,anak,satkerja,unit,golongan,masa_tahun,masa_bulan,eselon,kdbantu,gapok,tstruk,tfung,
        ...
        echo 'Proses Gaji 14 Pegawai P3K Bulan '.$bulan.' Tahun '.$thn.' Berhasil';	
 	}
