...
class M_Pegawai_gaji13 extends CI_Model {
	...
	public function load_header($key) {
		...
		$sql = "SELECT count(*) as tot from public.pegawai_13 a $where" ;
        $query1 = $this->db->query($sql);
        $total = $query1->row();
		
        $sql = "SELECT a.*,b.nm_satkerja,(select nm_unit from unitkerja where satkerja=a.satkerja and unit=a.unit) as nm_unit,
        (select nm_golongan from golongan where golongan=a.golongan) as nama_golongan from public.pegawai_13 a 
	...
	function saveData($post){
		...
		try {			
				...
				if($del){
					...
					$query = "INSERT INTO public.pegawai_13(nip_lama,nip,nama,nokartu,kota,agama,stskawin,anak,satkerja,unit,golongan,masa_tahun,masa_bulan,eselon,kdbantu,gapok,tstruk,tfung,
