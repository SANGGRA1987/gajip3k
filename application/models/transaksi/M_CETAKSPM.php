...
class M_cetakspm extends CI_Model {
	...
	public function getspm()
	{
		...
		$query 	= $this->db->query("select a.no_spm,a.tgl_spm from transaksi.trhspm a inner join public.satkerja b on left(a.kd_skpd,7)=b.satkerja order by a.no_spm");
		//$query 	= $this->db->query("select a.no_spm,a.tgl_spm from transaksi.trhspm a order by a.no_spm");
		$n = 0;
		if ($query->result() > 0) 
		{
			foreach ($query->result() as $key) {
	...
	public function cetakspm($param)
	{
