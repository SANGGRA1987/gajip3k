...
$(document).ready(function() {
	...
	function load_detail_pot(no_spm,kd_skpd) {
		...
		var kode = kode1;
        $('#dg1').datagrid({
            url:'<?php echo site_url('transaksi/C_SPM/load_detail_pot') ?>',
            idField:'id',
            width: '1000',
            height: '250',
            nowrap: true,
	...
	function get_total(no_spm,kd_skpd){
			$.ajax({
                    type: "POST",
                    url: '<?php echo base_url(); ?>transaksi/C_SPM/ambil_total',
                    dataType:"json",
	...
	function savedetail(){  
		...
		if(cbln=='01'){
			...
			$.ajax({
                    type: "POST",
                    url: '<?php echo base_url(); ?>transaksi/C_SPM/untuk',
                    dataType:"json",
                    ...
                    success:function(data){
						...
						var cuntuk = 'Untuk Pembayaran Gaji Bulan '+nm_bln+' '+cthn+' bagi '+cjumpeg+' Pegawai/ '+cjiwa+' Jiwa ('+cjumpeg+','+cjumnikah+','+cjumanak+') = '+cjiwa+' jiwa.  Pada '+nm_satkerja+' Sesuai SPP Nomor : terlampir. ';   
                        $("#untuk").val(cuntuk);
				...
				$.ajax({
                    type: "POST",
                    url: '<?php echo base_url(); ?>transaksi/C_SPM/rekening',
                    dataType:"json",
                    data:({satkerja:satkerja,gol1:gol1,gol2:gol2}),
                    success:function(data){
							$.each(data,function(i,n){ 	
	...
	window.onload = function(){
		...
		$("#no_spm1").textbox('setValue','.GJ/2021');
		$("#para_pegawai").textbox('setValue','Y');
		$("#golongan1").textbox('setValue','1');
		$("#golongan2").textbox('setValue','17');
		$("#j_belanja").textbox('setValue','2');
