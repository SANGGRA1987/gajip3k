...
$(document).ready(function() {
           $('#satkerja').combogrid({
				...
				idField:'satkerja',  
				textField:'satkerja',  
				url:'<?php echo base_url(); ?>transaksi/C_SPM_gaji14/get_satker', 
				mode:'remote',
				columns:[[  
			...
			$('#golongan1').combogrid({
				...
				idField:'id',  
				textField:'id',  
				url:'<?php echo base_url(); ?>transaksi/C_SPM_gaji14/golongan1', 
				mode:'remote',
				columns:[[  
	...
	function load_detail(no_spm,kd_skpd) {
		...
		var kode = kode1;
        $('#dg').datagrid({
            url:'<?php echo site_url('transaksi/C_SPM_gaji14/load_detail') ?>',
            idField:'id',
            width: '1000',
            height: '400',
            nowrap: true,
	...
	function load_detail_pot(no_spm,kd_skpd) {
		...
		var kode = kode1;
        $('#dg1').datagrid({
            url:'<?php echo site_url('transaksi/C_SPM_gaji14/load_detail_pot') ?>',
            idField:'id',
            width: '1000',
            height: '250',
            nowrap: true,
	...
	function get_total(no_spm,kd_skpd){
			$.ajax({
                    type: "POST",
                    url: '<?php echo base_url(); ?>transaksi/C_SPM_gaji14/ambil_total',
                    dataType:"json",
	...
	function savedetail(){  
		...
		if(cbln=='01'){
			...
			$.ajax({
                    type: "POST",
                    url: '<?php echo base_url(); ?>transaksi/C_SPM_gaji14/untuk',
                    dataType:"json",
                    ...
                    success:function(data){
						...
						var cuntuk = 'Untuk Pembayaran Gaji Bulan gaji ke 14 '+cthn+' bagi '+cjumpeg+' Pegawai/ '+cjiwa+' Jiwa ('+cjumpeg+','+cjumnikah+','+cjumanak+') = '+cjiwa+' jiwa.  Pada '+nm_satkerja+' Sesuai SPP Nomor : terlampir. ';   
                        $("#untuk").val(cuntuk);
				...
				$.ajax({
                    type: "POST",
                    url: '<?php echo base_url(); ?>transaksi/C_SPM_gaji14/rekening',
                    dataType:"json",
                    data:({satkerja:satkerja,gol1:gol1,gol2:gol2}),
                    success:function(data){
							$.each(data,function(i,n){ 	
	...
	window.onload = function(){
		...
		$("#no_spm1").textbox('setValue','.GJ14/2021');
		$("#para_pegawai").textbox('setValue','Y');
		$("#golongan1").textbox('setValue','1');
		$("#golongan2").textbox('setValue','17');
		$("#j_belanja").textbox('setValue','2');
    ...
    function back() {
        localStorage.clear();
        window.location.href = "<?php echo base_url(); ?>transaksi/C_SPM_gaji14";
    }
