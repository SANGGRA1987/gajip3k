...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('#dg').datagrid({
			...
			fitColumns:true,
			pagination:true,
			url: '<?php echo base_url(); ?>transaksi/C_SPM_gaji14/load_header',
	...
	function detail() {		
		...
		window.location.href = '<?php echo site_url('transaksi/C_SPM_gaji14/add'); ?>';
	}
	...
	function hapus() {
		...
		if ( row ){
			$.messager.confirm('Konfirmasi','Yakin ingin menghapus NO SPM  '+no_spm+' ?',function(r){
				if (r){
					$.post('<?php echo base_url(); ?>transaksi/C_SPM_gaji14/hapus',
